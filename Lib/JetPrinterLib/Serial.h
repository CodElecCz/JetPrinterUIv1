...
{	
	namespace JetPrinter
	{
		...
		public:
			Serial(void);
			Serial(const char* COMPort, BaudRate Baudrate = BR_9600, DataBits Databits = DB_8, Parity Parity = P_NONE, StopBits Stopbit = SB_ONE);
			~Serial(void);
		...
		private:
			...
			void ols_io_abort_reset();
			void ols_io_abort_pending();
