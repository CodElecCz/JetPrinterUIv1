...
#define COMM_EVTs	ARRAY_CNT(gs_comm_evt)
		...
		static const char* GetLastErrorString()
		{
			...
			if (dwLastError != 0) //system can not find file specified
			{
				FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM,                 // It´s a system error
					...
					wcslen(lpBuffer) - 1,                     // Number of bytes to store the message
					NULL);
			}

			wstring wstrBuffer(lpBuffer);
			size_t f = wstrBuffer.find(L"\r\n");
			if (f > 0)
				wstrBuffer.replace(f, std::wstring(L"\r\n").length(), L"");
		...
		void Serial::Connect(const char* COMPort, BaudRate Baudrate, DataBits Databits, Parity parity, StopBits Stopbits)
		{
			...
			{
				char err_msg[256];
				sprintf_s(err_msg, sizeof(err_msg), "CreateFile() error: %d; %s", GetLastError(), GetLastErrorString());
				throw err_msg;
			}
			m_Fh = fh;
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				sprintf_s(err_msg, sizeof(err_msg), "GetCommState() error: %d; %s", GetLastError(), GetLastErrorString());
				throw err_msg;
			}
			...
			if (succeed == FALSE)
			{
				...
				char	err_msg[256];
				sprintf_s(err_msg, sizeof(err_msg), "SetCommState() error: %d; %s", GetLastError(), GetLastErrorString());
				throw err_msg;
			}
			m_Dcb = dcb;
			...
			if (!m_Overlapped.ReadWriteFile.hEvent)
			{
				...
				char	err_msg[256];
				sprintf_s(err_msg, sizeof(err_msg), "CreateEvent() error: %d; %s", GetLastError(), GetLastErrorString());
				throw err_msg;
			}
		...
		void Serial::read(UINT8 ReadData[], PUINT32 ReadCount)
		{
			...
			else
			{
				...
				do
				{
					...
					{
						char err_msg[256];
						sprintf_s(err_msg, sizeof(err_msg), "Read() buffer full (%d bytes), no termination char received", ReadDataSize);
						throw err_msg;
					}
					...
					if (((clock() - start_clock) / (double)CLOCKS_PER_SEC) > ((double)m_Timeout / 1000.0))
					{
						char err_msg[256];
						if (readCount == 0)
							sprintf_s(err_msg, sizeof(err_msg), "Read() timeout: %d [ms]; no data received", m_Timeout);
						else
							sprintf_s(err_msg, sizeof(err_msg), "Read() timeout: %d [ms]; no termination char received", m_Timeout);
		...
		void Serial::read(UINT8 ReadData[], PUINT32 ReadCount, UINT32 ReadCountRequest)
		{
			...
			do
			{
				...
				{
					char err_msg[256];
					sprintf_s(err_msg, sizeof(err_msg), "Read() timeout: %d [ms]", m_Timeout);
					throw err_msg;
				}
		...
		void Serial::serial_read(UINT8 RadData[], PUINT32 ReadCount)
		{
			...
			do
			{
				...
				if (!succeed)
				{
					...
					{
						char err_msg[256];
						sprintf_s(err_msg, sizeof(err_msg), "ReadFile() error: %d; %s", GetLastError(), GetLastErrorString());
						throw err_msg;
					}
					}
				}
		...
		void Serial::serial_write(UINT8 WriteData[], UINT32 WriteCount)
		{
			...
			if (!succeed)
			{
				...
				{
					char err_msg[256];
					sprintf_s(err_msg, sizeof(err_msg), "WriteFile() error: %d; %s", GetLastError(), GetLastErrorString());
					throw err_msg;
				}
				}
			}
		...
		void Serial::ols_wait_for_overlapped(LPOVERLAPPED p_overlapped, DWORD *p_len, DWORD timeout)
		{
			...
			case WAIT_OBJECT_0:
				...
				{
					char err_msg[256];
					sprintf_s(err_msg, sizeof(err_msg), "GetOverlappedResult() error: %d; %s", GetLastError(), GetLastErrorString());
					throw err_msg;
				}
				break;
			case WAIT_TIMEOUT:
			{
				char err_msg[256];
				sprintf_s(err_msg, sizeof(err_msg), "WaitForSingleObject() timeout: %d [ms]", timeout);
				throw err_msg;
			}
			break;
			case WAIT_FAILED:
			{
				char err_msg[256];
				sprintf_s(err_msg, sizeof(err_msg), "WaitForSingleObject() failed: %d; %s", GetLastError(), GetLastErrorString());
				throw err_msg;
			}
			break;
			default:
			{
				char err_msg[256];
				sprintf_s(err_msg, sizeof(err_msg), "WaitForSingleObject() unknown return value: %d; error: %d; %s", ret, GetLastError(), GetLastErrorString());
				throw err_msg;
			}
			break;
			}
		...
		void Serial::ols_wait_comm_event(DWORD *pmask, UINT32 timeout)
		{
			...
			if (!succeed)
			{
				...
				{
					char err_msg[256];
					sprintf_s(err_msg, sizeof(err_msg), "WaitCommEvent() error: %d; %s", GetLastError(), GetLastErrorString());
					throw err_msg;
				}
				}
			}
